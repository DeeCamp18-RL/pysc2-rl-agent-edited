...
def fully_conv(config):
    ...
    for dim, is_spatial in config.policy_dims():
        if is_spatial:
            logits = layers.conv2d(state, num_outputs=1, kernel_size=1, activation_fn=None, data_format="NHWC")
...
def cnn_block(sz, dims, embed_dim_fn):
    ...
    for i, d in enumerate(dims):
        if d > 1:
            ...
            block[i] = layers.conv2d(block[i], num_outputs=embed_dim_fn(d), kernel_size=1, data_format="NHWC")
        else:
